...
def Schedule(sagerDict, overide, app):
    ...
    genrList = [hour % int(app.config['System']['SagerInterval']) for hour in hourList]
...
def Generate(sagerDict, app):
    ...
    # Get device ID
    if app.config['Station']['TempestID']:
        device = app.config['Station']['TempestID']
    elif app.config['Station']['SkyID']:
        device = app.config['Station']['SkyID']
    ...
    if app.config['Station']['TempestID']:
        Obs = {}
        getTempestData(Obs, Now, app.config)
        if not Obs:
    ...
    if app.config['Station']['OutAirID']:
        Obs = {}
        getAirData(Obs, Now, app.config)
        if not Obs:
    ...
    if np.all(np.isnan(Pres6)) or np.all(np.isnan(Pres)):
        ...
        sagerDict['Pres']  = derive.SLP([np.nanmean(Pres).tolist(), 'mb'], device, app.config)[0]
    ...
    # --------------------------------------------------------------------------
    Data = requestAPI.checkWX.METAR(app.config)
...
def getTempestData(Obs, Now, Config):
    ...
    if requestAPI.weatherflow.verifyResponse(Data, 'obs'):
        Obs['Time']    = [item[0] if item[0]   is not None else NaN for item in Data.json()['obs']]
        Obs['WindSpd'] = [item[2] if item[2]   is not None else NaN for item in Data.json()['obs']]
        Obs['WindDir'] = [item[4] if item[4]   is not None else NaN for item in Data.json()['obs']]
        Obs['Pres']    = [item[6] if item[7]   is not None else NaN for item in Data.json()['obs']]
        Obs['Temp']    = [item[6] if item[7]   is not None else NaN for item in Data.json()['obs']]
        Obs['Rain']    = [item[12] if item[12] is not None else NaN for item in Data.json()['obs']]
...
def getSkyData(Obs, Now, Config):
    ...
    if requestAPI.weatherflow.verifyResponse(Data, 'obs'):
        Obs['Time']    = [item[0] if item[0] is not None else NaN for item in Data.json()['obs']]
        Obs['WindSpd'] = [item[5] if item[5] is not None else NaN for item in Data.json()['obs']]
        Obs['WindDir'] = [item[7] if item[7] is not None else NaN for item in Data.json()['obs']]
        Obs['Rain']    = [item[3] if item[3] is not None else NaN for item in Data.json()['obs']]
...
def getAirData(Obs, Now, Config):
    ...
    if requestAPI.weatherflow.verifyResponse(Data, 'obs'):
        Obs['Time'] = [item[0] if item[0] is not None else NaN for item in Data.json()['obs']]
        Obs['Pres'] = [item[1] if item[1] is not None else NaN for item in Data.json()['obs']]
        Obs['Temp'] = [item[2] if item[2] is not None else NaN for item in Data.json()['obs']]
