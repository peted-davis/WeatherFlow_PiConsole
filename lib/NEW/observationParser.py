...
device_obs = {
    ...
    'dailyRain':    [None, 'mm'],    'obTime':       [None, 's'],       'minPres': [None, 'mb'],
    'maxPres':      [None, 'mb'],    'minTemp':      [None, 'c'],       'maxTemp': [None, 'c'],
...
class obsParser():
    ...
    def parse_obs_st(self, message, config):
        ...
        if (self.apiData[device_id]['flagAPI']
                or self.deviceObs['minPres'][0] is None
                or self.deviceObs['maxPres'][0] is None
                or self.deviceObs['minTemp'][0] is None
                or self.deviceObs['maxTemp'][0] is None
    ...
    def calcDerivedVariables(self, config, device_type):
        ...
        if device_type in ('obs_air', 'obs_st', 'obs_all'):
            ...
            self.deviceObs['outTempMax']   = derive.tempMax(self.deviceObs['outTemp'], self.deviceObs['obTime'], self.deviceObs['maxTemp'], self.apiData, config)
            self.deviceObs['outTempMin']   = derive.tempMin(self.deviceObs['outTemp'], self.deviceObs['obTime'], self.deviceObs['minTemp'], self.apiData, config)
            ...
            self.deviceObs['SLPMax']       = derive.SLPMax(self.deviceObs['pressure'], self.deviceObs['obTime'], self.deviceObs['maxTemp'], self.apiData, config)
            self.deviceObs['SLPMin']       = derive.SLPMin(self.deviceObs['pressure'], self.deviceObs['obTime'], self.deviceObs['minTemp'], self.apiData, config)
        ...
        if device_type in ('obs_air', 'obs_st', 'obs_all'):
            self.displayObs['Temp']          = observation.Format(outTemp,        'Temp')
            self.displayObs['FeelsLike']     = observation.Format(feelsLike,      'Temp')
            self.displayObs['DewPoint']      = observation.Format(dewPoint,       'Temp')
            self.displayObs['tempDiff']      = observation.Format(outTempDiff,    'Temp')
            self.displayObs['tempTrend']     = observation.Format(outTempTrend,   'Temp')
            self.displayObs['TempMax']       = observation.Format(outTempMax,     'Temp')
            self.displayObs['TempMin']       = observation.Format(outTempMin,     'Temp')
            self.displayObs['Humidity']      = observation.Format(humidity,       'Humidity')
            self.displayObs['Pres']          = observation.Format(SLP,            'Pressure')
            self.displayObs['PresTrend']     = observation.Format(SLPTrend,       'Pressure')
            self.displayObs['MaxPres']       = observation.Format(SLPMax,         'Pressure')
            self.displayObs['MinPres']       = observation.Format(SLPMin,         'Pressure')
