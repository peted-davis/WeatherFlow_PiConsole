...
def SunriseSunset(astroData, Config):
    ...
    astroData['Sunset'][2]  = sunsetMidnight / 86400
    astroData['Dusk'][2]    = duskMidnight / 86400
...
def sunTransit(astroData, Config, *largs):
    ...
    # If time after dusk, calculate new sunrise/sunset times and determine
    # number of nighttime hours remaining
    elif Now.replace(microsecond=0) >= astroData['Dusk'][0]:
    ...
    if astroData['Reformat'] and Now.date() == astroData['Sunrise'][0].date():
        Format(astroData, Config, 'Sun')
