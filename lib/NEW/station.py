...
class Station(Widget):
    ...
    def get_observationCount(self):
        ...
        if self.app.config['Station']['AirID']:
            urlList.append(Template.format(self.app.config['Station']['AirID'],     startTime, endTime, self.app.config['Keys']['WeatherFlow']))
        for URL in urlList:
            UrlRequest(URL,
    ...
    def parse_observationCount(self, request, response):
        ...
        if 'Station' in self.app.config:
            if 'obs' in response and response['obs'] is not None:
                ...
                elif str(response['device_id']) == self.app.config['Station']['AirID']:
    ...
    def fail_observationCount(self, request, response):
        ...
        elif deviceID == self.app.config['Station']['AirID']:
