""" Defines the Websocket service required by the Raspberry Pi Python console
...
class websocketClient():
    ...
    def __init__(self):
        ...
        # received to stop Websocket service or switch devices
        while self._keep_running:
            message = self.async_loop.run_until_complete(self.__async__getMessage())
            self.async_loop.run_until_complete(self.__async__decodeMessage(message))
            if self._switch_device:
                print('SWITCH')
                self.async_loop.run_until_complete(self.__async__manageDevices('listen_stop'))
                config.switch(self.stationMetaData, self.deviceList, self.config)
                self.obsParser.flagAPI = [1, 1, 1, 1]
                self.async_loop.run_until_complete(self.__async__manageDevices('listen_start'))
                self._switch_device = False
    ...
    def stopWebsocket(self, payload):
